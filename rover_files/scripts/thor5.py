...
# List of authorised plates (example, modify as needed)
AUTHORISED_PLATES = {'MH20DV2366', 'GJ01AB1234', 'DL05XY7890'}
...
def clean_ocr_text(text):
    ...
    # Remove unwanted special characters, but allow hyphens temporarily
    cleaned = re.sub(r'[^A-Z0-9-]', '', raw)
    # Remove hyphens for validation
    cleaned_no_hyphen = cleaned.replace('-', '')
    # Try the cleaned text first
    if re.fullmatch(PLATE_PATTERN, cleaned_no_hyphen):
        return cleaned_no_hyphen
    # Apply corrections for common OCR errors
    corrected = cleaned_no_hyphen.replace('O', '0').replace('I', '1').replace('Z', '2').replace('S', '5').replace('B', '8')
    if re.fullmatch(PLATE_PATTERN, corrected):
        return corrected
    # If still invalid, try extracting only valid characters and check length
    filtered = ''.join(c for c in corrected if c.isalnum())
    if len(filtered) >= 8 and re.fullmatch(PLATE_PATTERN, filtered[:10]):
        return filtered[:10]
    return None
...
def detection_loop():
    ...
    try:
        ...
        inference = ModelInference(loader, conf_threshold=0.5, iou_threshold=0.45)
                ...
                if x2 > x1 and y2 > y1:
                    ...
                    if crop.size != 0:
                        ...
                        if text:
                            ...
                            if cleaned:
                                status = 'authorised' if cleaned in AUTHORISED_PLATES else 'unauthorised'
                                try:
                                    db.upsert_number_plate(cleaned, slot, status=status)
                                    print(f"[DB] ✅ Upserted plate: {cleaned}, Slot: {slot}, Status: {status}")
                                except Exception as e:
                                    print(f"[DB] ❌ Error upserting plate: {e}")
                                plate_texts.append(f"{cleaned} (Slot {slot}, Status {status}, Conf {pscores[j]:.2f})")
...
def get_valid_command():
    valid_commands = {'f', 'b', 's', 'l', 'r', 'u', 'forward', 'backward', 'stop', 'left', 'right', 'uturn'}
    while True:
        try:
            cmd = input("Enter command (f=forward, b=backward, s=stop, l=left, r=right, u=uturn): ").strip().lower()
            command_map = {
                'forward': 'f',
                'backward': 'b',
                'stop': 's',
            ...
            if cmd in valid_commands:
                return cmd
            print("❌ Invalid command. Use f, b, s, l, r, u or their full words.")
        except EOFError:
            print("\n[DEBUG] EOF detected. Stopping rover...")
            return 's'

def get_valid_throttle():
    while True:
        try:
            throttle_input = input("Throttle % (0–100): ").strip()
            throttle = int(throttle_input)
            if 0 <= throttle <= 100:
                return throttle
            print("Throttle must be between 0 and 100")
        except ValueError:
            print("Invalid throttle input. Please enter a number between 0 and 100.")
        except EOFError:
            print("\n[DEBUG] EOF detected. Stopping rover...")
            return 0
...
def main():
    ...
    try:
        ...
        while time.time() - start_time < DURATION and not should_stop:
            cmd = get_valid_command()
            if cmd == 's':
                stop_rover(vehicle)
                should_stop = True  # Signal detection thread to exit
                break
            elif cmd in ['f', 'b']:
                throttle = get_valid_throttle()
                if throttle == 0:  # Handle EOF in throttle input
                    stop_rover(vehicle)
                    should_stop = True
                    break
                move_rover(vehicle, cmd, throttle)
                ...
                spin_rover(vehicle, 'left', 1)
            elif cmd == 'r':
                spin_rover(vehicle, 'right', 1)
            elif cmd == 'u':
                spin_rover(vehicle, 'right', 0.6)
            if drift_threshold is not None:
                steering = adjust_steering_for_yaw(vehicle, initial_yaw, drift_threshold)
                send_rc_override(vehicle, current_throttle, steering)
            time.sleep(0.01)  # Prevent CPU overload

        print("\n[DEBUG] Movement duration ended or stopped; stopping rover...")
        stop_rover(vehicle)
        ...
        if detection_thread.is_alive():
            detection_thread.join(timeout=2.0)  # Wait for detection thread to exit
        if vehicle and db:
            try:
        ...
        if plate_to_slot:
            for plate, slot in plate_to_slot.items():
                status = 'authorised' if plate in AUTHORISED_PLATES else 'unauthorised'
                print(f"  • Car with plate {plate} in Slot {slot} ({status})")
        else:
